...
def _cull_interior(
    ...
    Remove:
      ...
      • Exterior items (outside all kept room polygons)
    """
...
def _cull_exterior(
    ...
    Keep:
      ...
      • ALL holes (doors/windows) — their openings show as BLACK from outside
        because no interior geometry is rendered behind them (intentional)
      • Items physically OUTSIDE all room polygons (garden, trees, etc.)
      • Floor assets always

    Remove:
      • ALL interior furniture/items that sit INSIDE any room polygon
        (sofas, fridges, lights, etc. — invisible from outside)
      • Door/window decorative assets that are inside a room are also culled
        unless they are tagged with a line/hole reference (architectural fittings)

    Window/door openings appear BLACK by design: no interior backdrop is
    generated. The renderer will show void/black through any wall hole when
    the camera is outside. This is the correct behaviour.

    Texture note: only exterior-facing textures should be applied on walls
    in this mode. That is handled by the Godot/Blender material system using
    the existing wall data — no extra flags are set here.
    """
    ...
    for iid, item in items.items():
        ...
        # Is the item outside all room polygons? → external landscaping → keep
        ...
        # Item is INSIDE a room polygon.
        # Only keep it if it is an architectural wall/door/window fitting
        # (has an explicit line or hole reference — means it is part of wall geometry).
        is_wall_fitting = bool(item.get("line") or item.get("hole"))

        # Secondary check: is it very close (<= 30 cm) to a wall that has holes?
...
class SceneOptimizer:
    ...
    def cull_scene(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """
        Optimise the render payload using two-mode culling:
