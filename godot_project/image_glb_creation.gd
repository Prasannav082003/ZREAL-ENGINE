	...
	if data.has("layers"):
		...
		for layer_id in data["layers"]:
			...
			if layer.has("lines") and layer.has("vertices"):
				_build_layer_geometry(layer, layer_alt, show_all_floors)
	elif data.has("lines") and data.has("vertices"):
		print("Found lines/vertices in root data.")
		_build_layer_geometry(data, 0.0, data.get("showAllFloors", true))
	else:
		print("Warning: No lines or vertices found in floor plan data.")

func _build_layer_geometry(layer_data, layer_altitude, show_all_floors = true):
	...
	if layer_data.has("areas"):
		...
		for area_id in layer_data["areas"]:
			...
			# explicitly marked visible by the scene_optimizer (camera inside).
