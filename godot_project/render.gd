...
func render_image(data, output_path):
	...
	if cam:
		# setup_fixed_fill_lights() — DISABLED: these per-room OmniLights placed
		# at each area centroid were the source of the persistent center-room
		# brightness blob.  Fixture OmniLights from build_scene() are sufficient.
...
func render_video(data, output_base_path):
	...
	# setup_fixed_fill_lights() — DISABLED (same as render_image)
