...
def save_asset_report(output_glb_path):
    """Saves the detailed asset processing report to a JSON file side-by-side with the GLB."""
    try:
        folder = os.path.dirname(os.path.abspath(output_glb_path))
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            json.dump(ASSET_PROCESSING_REPORT, f, indent=2)
        log(f"✅ Saved detail asset report to: {report_path}")
    except Exception as e:
        ...
        for mat_slot_idx, mat_slot in enumerate(obj.data.materials):
            ...
            if manual_mat_data and has_manual_changes(manual_mat_data):
                ...
                if manual_mat_data.get('color'):
                    ...
                    report_entry["applied_texture"] = map_path
...
def main():
    ...
    for layer in all_layers:
        ...
        # Calculate Layer Altitude in Meters
        layer_altitude_cm = get_dimension_value(layer.get('altitude', 0), default=0)
        layer_altitude = layer_altitude_cm * SCALE
        
        is_top_floor = (layer_altitude_cm >= max_altitude)
