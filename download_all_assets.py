...
def relative_to_full_url(rel_path: str) -> str:
    ...
    IMPORTANT: The API sometimes returns already-percent-encoded paths
    (e.g. 'glb-assets/Lamp%20(2)/file.glb'). We decode first so that
    urllib.parse.quote can re-encode consistently, preventing the
    double-encoding that causes Azure Blob Storage to return HTTP 404.
    """
    from urllib.parse import unquote, quote
    rel_path = rel_path.strip()
    if rel_path.startswith("http"):
        # Already a full URL — decode + re-encode path segment only
    ...
    # Strip any accidental leading slash then decode → re-encode
    rel_path = rel_path.lstrip("/")
    rel_path = quote(unquote(rel_path), safe="/")  # normalise encoding
...
def extract_asset_items(assets: list) -> list[tuple[str, str, str]]:
    ...
    for rec in assets:
        ...
        name    = rec.get("asset3D_Name", "?")

        # Primary: prefer highPoly_Glb when available
        if hp_rel.strip():
            hp_url = relative_to_full_url(hp_rel.strip())
            items.append((hp_url, hp_url, f"Asset [{name}]  HP=True"))

            # Fallback: also queue glb_Url as a separate entry that will
            # only be attempted if the HP URL fails (different URL = different
            # key in url_map, so it won't be double-downloaded on success).
            # We mark it so the consumer can identify it as a fallback.
            if glb_rel.strip() and glb_rel.strip() != hp_rel.strip():
                glb_url = relative_to_full_url(glb_rel.strip())
                if glb_url != hp_url:  # different blob path
                    items.append((glb_url, glb_url,
                                  f"Asset [{name}]  HP=False (fallback for HP)"))

        elif glb_rel.strip():
            glb_url = relative_to_full_url(glb_rel.strip())
            items.append((glb_url, glb_url, f"Asset [{name}]  HP=False"))
        else:
            skipped_no_url += 1
            continue
